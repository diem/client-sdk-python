 ...
 */
enum LibraStatus libra_LibraAccountResource_from(
    const uint8_t* buf,
    size_t len,
    struct LibraAccountResource* out);

/*!
 *  Get serialized signed transaction from a list of transaction parameters
 *
 * To get the serialized transaction in a memory safe manner, the client needs
 * to pass in a pointer to a pointer to the allocated memory in rust and call
 * free on the memory address with `libra_free_bytes_buffer`.
 * @param[in] sender_private_key is sender's private key
 * @param[in] sequence is the sequence number of this transaction corresponding
 * to sender's account.
 * @param[in] max_gas_amount is the maximal total gas specified by wallet to
 * spend for this transaction.
 ...
 * @param[in] expiration_timestamp_secs is the time this TX remain valid, the
 ...
 * @param[out] ptr_buf is the pointer that will be filled with the memory
 * address of the transaction allocated in rust. User takes ownership of pointer
 * returned by *buf, which needs to be freed using libra_free_bytes_buffer
...
enum LibraStatus libra_SignedTransactionBytes_from(
    const uint8_t sender_private_key[LIBRA_PRIVKEY_SIZE],
    uint64_t sequence,
    uint64_t max_gas_amount,
    uint64_t gas_unit_price,
    const char* gas_identifier,
 ...
 *
 * To get the serialized script in a memory safe manner, the client needs to
 * pass in a pointer to a pointer to the allocated memory in rust and call free
 * on the memory address with `libra_free_bytes_buffer`.
 ...
 * @param[in] metadata_signature_bytes is the metadata signature bytes for given
 * transaction.
 * @param[in] metadata_signature_len is the length of metadata_signature_bytes
 * array.
 * @param[out] ptr_buf is the pointer that will be filled with the memory
 * address of the script allocated in rust. User takes ownership of pointer
 * returned by *buf, which needs to be freed using libra_free_bytes_buffer
 * @param[out] ptr_len is the length of the script memory buffer.
 */
enum LibraStatus libra_TransactionP2PScript_from(
    const uint8_t receiver[LIBRA_ADDRESS_SIZE],
 ...
 * @param[out] ptr_len is the length of the script memory buffer.
 */
enum LibraStatus libra_TransactionAddCurrencyScript_from(
    const char* identifier,
    uint8_t** ptr_buf,
    size_t* ptr_len);
 ...
 *  Encode a program that rotates `vasp_root_addr`'s base URL to `new_url` and
 * compliance public key to `new_key`.
 *
 * To get the serialized script in a memory safe manner, the client needs to
 * pass in a pointer to a pointer to the allocated memory in rust and call free
 * on the memory address with `libra_free_bytes_buffer`.
 ...
 * @param[out] ptr_len is the length of the script memory buffer.
 */
enum LibraStatus libra_TransactionRotateDualAttestationInfoScript_from(
    const uint8_t* new_url_bytes,
    size_t new_url_len,
    const uint8_t new_key_bytes[LIBRA_PUBKEY_SIZE],
    uint8_t** ptr_buf,
    size_t* ptr_len);

/*!
 * Function to free the allocation memory in rust for bytes
 * @param buf is the pointer to the bytes allocated in rust, and needs to be
 * freed from client side
 */
 ...
 * @returns status code, one of LibraAPIStatus
 */
enum LibraStatus libra_LibraSignedTransaction_from(
    const uint8_t* buf,
    size_t len,
    struct LibraSignedTransaction* out);

/*!
 * Get serialized raw transaction from a list of transaction parameters
 *
 * To get the serialized raw transaction in a memory safe manner, the client
 * needs to pass in a pointer to a pointer to the allocated memory in rust and
 * call free on the memory address with `libra_free_bytes_buffer`.
 ...
 * spend for this transaction.
 * @param[in] gas_unit_price is the maximal price can be paid per gas.
 * @param[in] expiration_time_secs is the time this TX remain valid, the format
 * is unix timestamp.
 * @param[in] chain_id is the chain id for this Transaction.
 * @param[in] metadata_bytes is the metadata bytes for given transaction.
 * @param[in] metadata_len is the length of metadata_bytes array.
 * @param[out] buf is the pointer that will be filled with the memory address of
 * the transaction allocated in rust. User takes ownership of pointer returned
 * by *buf, which needs to be freed using libra_free_bytes_buffer
 * @param[out] len is the length of the raw transaction memory buffer.
 */
enum LibraStatus libra_RawTransactionBytes_from(
    const uint8_t sender[LIBRA_ADDRESS_SIZE],
    const uint8_t receiver[LIBRA_ADDRESS_SIZE],
    uint64_t sequence,
    uint64_t num_coins,
    uint64_t max_gas_amount,
...
/*!
 * This function takes in a raw transaction, public key and signature in bytes,
 * and return a signed transaction in bytes. To get the serialized signed
 * transaction in a memory safe manner, the client needs to pass in a pointer to
 * a pointer to the allocated memory in rust and call free on the memory address
 * with `libra_free_bytes_buffer`.
 ...
 * @param[out] buf_result is the pointer that will be filled with the memory
 * address of the transaction allocated in rust. User takes ownership of pointer
 * returned by *buf, which needs to be freed using libra_free_bytes_buffer
 * @param[out] len_result is the length of the signed transaction memory buffer.
 */
enum LibraStatus libra_RawTransaction_sign(
    const uint8_t* buf_raw_txn,
    size_t len_raw_txn,
    const uint8_t* buf_public_key,
    size_t len_public_key,
    const uint8_t* buf_signature,
    size_t len_signature,
    uint8_t** buf_result,
    size_t* len_result);

/*!
 * This function takes in private key in bytes and return the associated public
 * key and address
 * @param[in] private_key_bytes is private key in bytes
 * @param[out] out is a pointer to LibraAccountKey struct client passed in by
 * initializing an empty LibraAccountKey struct
 */
enum LibraStatus libra_LibraAccountKey_from(
    const uint8_t private_key_bytes[LIBRA_PRIVKEY_SIZE],
    struct LibraAccountKey* out);

/*!
 * This function takes in an event key, event data and event type tag in bytes,
 * and return LibraEvent. To get the event in a memory safe manner, the client
 * needs to call free on the output with `libra_LibraEvent_free`.
 ...
 */
enum LibraStatus libra_LibraEvent_from(
    const uint8_t* buf_key,
    size_t len_key,
    const uint8_t* buf_data,
    size_t len_data,
    const uint8_t* buf_type_tag,
    size_t len_type_tag,
    struct LibraEvent** out);

/*!
 * Function to free the allocation event memory in rust
 * @param out is the pointer to the event struct allocated, and needs to be
 * freed from client side
 */
 ...
 * Error will be in UTF8 string encoding and client does not need to free the
 * string from their side.
