...
cdef extern from "data.h":
    ...
    struct LibraP2PTransferTransactionArgument:
        ...
        size_t metadata_len
        uint8_t* metadata_signature_bytes
    ...
    LibraStatus libra_TransactionRotateDualAttestationInfoScript_from(const uint8_t* new_url_bytes, size_t new_url_len, const uint8_t new_key_bytes[LIBRA_PUBKEY_SIZE], uint8_t **ptr_buf, size_t *ptr_len)
    ...
    LibraStatus libra_RawTransactionBytes_from(const uint8_t sender[LIBRA_ADDRESS_SIZE], const uint8_t receiver[LIBRA_PUBKEY_SIZE], uint64_t sequence, uint64_t num_coins, uint64_t max_gas_amount, uint64_t gas_unit_price, uint64_t expiration_time_secs, uint8_t chain_id, const uint8_t * metadata_bytes, size_t metadata_len, uint8_t** buf, size_t* len)
