...
class JsonRpcClient:
    ...
    def execute(self, batch: JsonRpcBatch) -> JsonRpcResponse:
        ...
        try:
            ...
            raise ClientError(f"Error in connecting to Full Node: {e}\nPlease retry...")
    ...
    def _extract_result_or_error(  # noqa [C901]
            ...
            if json_rpc_response["result"]:
                try:
                    tx_response = from_dict(
                        data_class=TransactionResponse,
                        data=json_rpc_response["result"],
                        config=Config(strict=True),
                    )
                except (
                ...
                if not isinstance(tx_response.transaction, UserTransaction):
                    raise InvalidServerResponse(
        ...
        if request_method == "get_transactions":
            ...
            try:
                transactions: typing.List[TransactionResponse] = []
