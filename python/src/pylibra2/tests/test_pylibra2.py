...
def get_valid_tx_response(
    ...
    ak = AccountKeyUtils.from_private_key(bytes.fromhex(sender_private_key))
    sender_address = ak.address.hex()

    valid_tx_resp["result"]["transaction"]["sender"] = sender_address
...
def test_account1() -> None:
    account_pk = bytes.fromhex("44" * 32)
    account_ak = AccountKeyUtils.from_private_key(account_pk)
    ...
    def _signed_transaction_bytes(type: str = "p2p"):
        if type == "p2p":
            ...
            mock_signed_tx_bytes = TransactionUtils.createSignedRotateDualAttestationInfoTransaction(
                new_url="https://client.testnet.libra.org",
...
def account_state_response():
    def _account_state_response(id: int):
        return {
            "id": id,
            "jsonrpc": "2.0",
            JSONRPC_LIBRA_CHAIN_ID: 2,
...
def test_mint_and_wait_success(
    mock_session, test_account1, transaction_response
) -> None:
    # Setting up Mock
    mock_session.post.return_value.ok = True
    mock_session.post.return_value.text = 0
    mock_session.post.return_value.json.return_value = transaction_response(1)

    libra_client = LibraClient(session=mock_session)
    libra_client.mint_and_wait(test_account1.authentication_key.hex(), 1_000_000, "LBR")

    mock_session.post.assert_called()


@patch("requests.Session")
def test_mint_and_wait_fail_faucet(mock_session, test_account1) -> None:
    mock_session.post.side_effect = RequestException
    with pytest.raises(ClientError):  # pyre-ignore
        libra_client = LibraClient(session=mock_session)
        libra_client.mint_and_wait(
            test_account1.authentication_key.hex(), 1_000_000, "LBR"
        )


@patch("requests.Session")
def test_mint_and_wait_fail_waiting(mock_session, test_account1) -> None:
...
def test_submit_transaction_and_wait_waiting_failure_status(
    ...
    invalid_tx_response["result"]["vm_status"] = -32006
...
@patch("requests.Session")
def test_get_account_success(mock_session, test_account1, account_state_response):

    mock_session.post.return_value.json.return_value = [account_state_response(0)]

    libra_client = LibraClient(session=mock_session)
    account = libra_client.get_account(test_account1.address.hex())

    mock_session.post.assert_called()
    assert isinstance(account, LibraAccount)

    # Check various fields
    assert account.address == test_account1.address.hex()
    assert (
    ...
    assert account.vasp_role == "parent_vasp"
    assert account.parent_vasp_info == {
        "human_name": "testnet",
        "base_url": "https://libra.org",
        "compliance_key": "00000000000000000000000000000000",
        "expiration_time": 18446744073709552000,
...
@patch("requests.Session")
def test_get_account_fail(mock_session, test_account1):
    mock_session.post.side_effect = RequestException

    with pytest.raises(ClientError):
        libra_client = LibraClient(session=mock_session)
        libra_client.get_account(test_account1.address.hex())


@patch("requests.Session")
def test_get_account_stale_resp_fail(mock_session, test_account1, stale_resp):
...
def test_p2p_tx_sucess(
    ...
    calls = [
        call(
            "https://client.testnet.libra.org/",
            json=[
                {
                    "jsonrpc": "2.0",
                    "id": 0,
...
def test_get_account_transaction_success(
    ...
    libra_client = LibraClient(session=mock_session)
    tx = libra_client.get_account_transaction(test_account1.address.hex(), 0, True)
    ...
    assert isinstance(tx.events[0], LibraSentPaymentEvent)


@patch("requests.Session")
def test_get_account_transaction_fail(mock_session, test_account1):
    mock_session.post.side_effect = RequestException

    with pytest.raises(ClientError):
        libra_client = LibraClient(session=mock_session)
        libra_client.get_account_transaction(test_account1.address.hex(), 0, True)


@patch("requests.Session")
def test_get_account_transaction_error_resp_fail(
    mock_session, test_account1, json_rpc_error_resp
):
...
def test_get_account_transaction_stale_resp_fail(
    mock_session, test_account1, stale_resp
):
...
def test_get_account_sent_events_success(
    mock_session, test_account1, account_state_response, multiple_events_response
):
    mock_session.post.side_effect = [
    ...
    libra_client = LibraClient(session=mock_session)
    events = libra_client.get_account_sent_events(test_account1.address.hex(), 0, 10)
            ...
            json=[
                {
                    ...
                    "id": 0,
                    "method": "get_account",
                    "params": [test_account1.address.hex()],
                }
            ],
            timeout=(DEFAULT_CONNECT_TIMEOUT_SECS, DEFAULT_TIMEOUT_SECS),
        ),
    ...
    assert len(events) == 2
    assert isinstance(events[0], LibraSentPaymentEvent)
    assert isinstance(events[1], LibraSentPaymentEvent)
...
@patch("requests.Session")
def test_get_account_sent_events_fail(mock_session, test_account1):
    mock_session.post.side_effect = RequestException

    with pytest.raises(ClientError):
        libra_client = LibraClient(session=mock_session)
        libra_client.get_account_sent_events(test_account1.address.hex(), 0, 10)


@patch("requests.Session")
def test_get_account_sent_events_error_resp_fail(
    mock_session, test_account1, account_state_response, json_rpc_error_resp
):
    mock_session.post.side_effect = [
        MagicMock(json=MagicMock(return_value=[account_state_response(0)])),
...
def test_get_account_received_events_success(
    ...
    libra_client = LibraClient(session=mock_session)
    events = libra_client.get_account_received_events(
        test_account1.address.hex(), 0, 10
    )
    ...
    assert len(events) == 2
    assert isinstance(events[0], LibraReceivedPaymentEvent)
    assert isinstance(events[1], LibraReceivedPaymentEvent)
...
@patch("requests.Session")
def test_get_account_received_events_fail(mock_session, test_account1):
    mock_session.post.side_effect = RequestException

    with pytest.raises(ClientError):
        libra_client = LibraClient(session=mock_session)
        libra_client.get_account_received_events(test_account1.address.hex(), 0, 10)
...
@patch("requests.Session")
def test_get_currencies_fail(mock_session, test_account1):
    mock_session.post.side_effect = RequestException
