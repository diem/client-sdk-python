...
from .. import libra_types, serde_types as st
from ..libra_types import (
    AccountAddress,
    Script,
    TransactionArgument,
...
def encode_add_currency_to_account_script(currency: TypeTag) -> Script:
    ...
    `Currency`.
    """
    return Script(code=ADD_CURRENCY_TO_ACCOUNT_CODE, ty_args=[currency], args=[])
...
def encode_create_recovery_address_script() -> Script:
    ...
    * Aborts with `RecoveryAddress::ENOT_A_VASP` if `account` is not a ParentVASP or ChildVASP
    """
    return Script(code=CREATE_RECOVERY_ADDRESS_CODE, ty_args=[], args=[])
...
def decode_add_currency_to_account_script(script: Script) -> ScriptCall:
    return ScriptCall__AddCurrencyToAccount(currency=script.ty_args[0])
...
def decode_burn_txn_fees_script(script: Script) -> ScriptCall:
    return ScriptCall__BurnTxnFees(coin_type=script.ty_args[0])
...
def decode_mint_lbr_script(script: Script) -> ScriptCall:
    return ScriptCall__MintLbr(amount_lbr=decode_u64_argument(script.args[0]))
...
def decode_unmint_lbr_script(script: Script) -> ScriptCall:
    return ScriptCall__UnmintLbr(amount_lbr=decode_u64_argument(script.args[0]))
...
def decode_update_minting_ability_script(script: Script) -> ScriptCall:
    return ScriptCall__UpdateMintingAbility(
        currency=script.ty_args[0], allow_minting=decode_bool_argument(script.args[0])
    )
