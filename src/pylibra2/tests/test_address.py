import pytest
from pylibra2 import LibraUserIdentifier
...
def test_encode_addr_success():
    ...
    enocded_addr = LibraUserIdentifier.encode(test_onchain_address, None, "lbr")
    assert enocded_addr == enocded_addr_with_none_subaddr

    # even with zero sub_address, expected should not change from above
    enocded_addr = LibraUserIdentifier.encode(test_onchain_address, zero_sub_address, "lbr")
    assert enocded_addr == enocded_addr_with_none_subaddr

    # test with some subaddress
    enocded_addr = LibraUserIdentifier.encode(test_onchain_address, test_sub_address, "lbr")
...
def test_encode_addr_fail():
    ...
    with pytest.raises(ValueError):
        LibraUserIdentifier.encode(test_onchain_address, None, "btc")

    # wrong subadress (length should be 8 bytes)
    with pytest.raises(ValueError):
        LibraUserIdentifier.encode(test_onchain_address, test_sub_address[:-2], "lbr")

    # wrong address (length should be 16 bytes)
    with pytest.raises(ValueError):
        LibraUserIdentifier.encode(test_onchain_address + "ff", test_sub_address[:-2], "lbr")


def test_decode_addr_success():
    # test enocded_addr_with_none_subaddr
    addr, subaddr = LibraUserIdentifier.decode(enocded_addr_with_none_subaddr, "lbr")
    assert addr == test_onchain_address
    assert subaddr is None

    # test enocded_addr_with_subaddr
    addr, subaddr = LibraUserIdentifier.decode(enocded_addr_with_subaddr, "lbr")
    assert addr == test_onchain_address
...
def test_decode_addr_fail():
    ...
    with pytest.raises(ValueError):
        LibraUserIdentifier.decode(invalid_hrp_encoded_address, "lbr")

    # fail to decode invalid "expected" hrp
    with pytest.raises(ValueError):
        LibraUserIdentifier.decode("lbr1p7ujcndcl7nudzwt8fglhx6wxn08kgs5tm6mz4usw5p72t", "tlb")

    # fail to decode invalid version
    invalid_version_encoded_address = "lbr1q7ujcndcl7nudzwt8fglhx6wxn08kgs5tm6mz4usw5p72t"  # v = 0
    with pytest.raises(ValueError):
        LibraUserIdentifier.decode(invalid_version_encoded_address, "lbr")
    ...
    with pytest.raises(ValueError):
        LibraUserIdentifier.decode(invalid_checksum_encoded_address, "lbr")
    ...
    with pytest.raises(ValueError):
        LibraUserIdentifier.decode(mixedcase_encoded_address)
    ...
    with pytest.raises(ValueError):
        LibraUserIdentifier.decode(short_encoded_address, "lbr")
    ...
    with pytest.raises(ValueError):
        LibraUserIdentifier.decode(large_encoded_address, "lbr")
    ...
    with pytest.raises(ValueError):
        LibraUserIdentifier.decode(invalid_separator_encoded_address, "lbr")
    ...
    with pytest.raises(ValueError):
        LibraUserIdentifier.decode(invalid_char_encoded_address, "lbr")
