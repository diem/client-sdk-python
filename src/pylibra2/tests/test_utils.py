...
def test_keys_addr():
    return {
        ...
        "public_key": bytes.fromhex(
            "664f6e8f36eacb1770fa879d86c2c1d0fafea145e84fa7d671ab7a011a54d509"
        ),
        "auth_key": bytes.fromhex(
            "231a656a51c1792efdb10f42ddbca0f80468de5bb622c235a681b898929cecf7"
        ),
...
def test_create_new_ed25519_key_pair(mock_private_key_generate, test_keys_addr):
    mock_private_key_generate.return_value = Mock(
        private_bytes=Mock(return_value=test_keys_addr["private_key"])
    )
...
def test_ed25519_public_key_from_private_key_success(test_keys_addr):
    public_key = utils.LibraCryptoUtils.ed25519_public_key_from_private_key(
        test_keys_addr["private_key"]
    )
...
def test_ed25519_public_key_from_private_key_fail(test_keys_addr):
    with pytest.raises(ValueError):
        utils.LibraCryptoUtils.ed25519_public_key_from_private_key(
...
def test_create_account_from_private_bytes_fail():
    with pytest.raises(ValueError):
        utils.LibraCryptoUtils.LibraAccount.create_from_private_key(
            bytes.fromhex("ABCDEF")
        )
